...
- Telop at top with slide-down + fade-in / fade-out animation
- Stylish xfade transitions varied per slide
"""
...
FPS  = 25
FADE = 0.8
DUR  = (30 + 7 * FADE) / 8  # ≈ 4.45s per slide → total 30s
...
def slide_filter(idx, main_text, sub_text):
    ...
    base = (f"[{idx}:v]scale={W}:{H}:force_original_aspect_ratio=increase,"
            f"crop={W}:{H},setsar=1")

    # Ken Burns (alternate zoom direction)
    z = f"'1+0.02*on/{total_frames}'" if idx % 2 == 0 else f"'1.04-0.02*on/{total_frames}'"
    zoompan = (f"zoompan=z={z}:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)'"
               f":d={total_frames}:s={W}x{H}:fps={FPS}")

    # --- Text animations ---
    # Alpha: fade-in over 0.7s, hold, fade-out over FADE
    ...
    # Y animation: slide down into position (top area)
    # main lines start slightly above final position
    MAIN_TOP   = int(H * 0.07)
    SUB_TOP    = int(H * 0.22)
    SLIDE_DIST = 40   # px slide distance

    # Decorative thin line between main and sub
    LINE_Y = int(H * 0.205)
    ...
    # Thin separator line (drawn as a zero-height wide rectangle via drawbox)
    draws.append(
        f"drawbox=x=(w-320)/2:y={LINE_Y}:w=320:h=2:color=white@0.6:t=fill"
        f":enable='if(lt(t,1.1),0,1)'"
    )

    # Sub text – slide up into position
    sub_final_y = SUB_TOP
    sub_y_expr  = (f"'if(lt(t,1.1),{sub_final_y+SLIDE_DIST},"
                   f"if(lt(t,1.7),{sub_final_y+SLIDE_DIST}-(t-1.1)/0.6*{SLIDE_DIST},{sub_final_y}))'")
    draws.append(
    ...
    return base + "," + zoompan + "," + ",".join(draws) + f"[v{idx}]"
...
def build():
    ...
    for i, (fname, main, sub, _) in enumerate(SLIDES):
        ...
        filters.append(slide_filter(i, main, sub))
